...
def evaluate_profile(report: dict) -> dict:
    ...
    if stop_p95 is None or attempts < MIN_INTERRUPTIONS:
        ...
        checks["stop_p95"] = "PASS" if stop_p95 <= MAX_STOP_P95_MS else "FAIL"

    quiet = int(barge.get("no_interruption_trials") or 0)
    false_stops = int(barge.get("false_stops") or 0)
    echo_commands = int(barge.get("echo_commands") or 0)
    if echo_commands > 0:
        checks["no_echo_commands"] = "FAIL"
    else:
        checks["no_echo_commands"] = "PASS" if quiet >= MIN_QUIET_TRIALS else "VERIFY"
    if false_stops > 0:
        checks["no_false_stops"] = "FAIL"
    else:
        checks["no_false_stops"] = "PASS" if quiet >= MIN_QUIET_TRIALS else "VERIFY"
    ...
    if int(streaming.get("duplicate_finals") or 0) > 0:
        ...
        # nessun partial pubblicato: accettabile solo come fallback dichiarato a frase completa
        checks["partials"] = "PASS" if streaming.get("fallback_full_utterance") else "VERIFY"
    else:
...
def evaluate_wake(reports: list[dict]) -> dict:
    ...
    return {"hours": round(hours, 2), "false_wakes": false_wakes, "intentional": intentional, "missed": missed,
            "checks": checks, "verdict": _combine(checks.values())}
...
def run_session(profile: str, notes: str = "") -> Path:
    ...
    report: dict = {
        ...
        "hardware": _hardware_description(session),
        "raw_audio_retained": False,
        "notes": notes,
        "measured_by_jake": measured,
    }
    if profile in AUDIO_PROFILES:
        attempts = _ask_int("Quante interruzioni hai provato a fare?")
        report["barge_in"] = {
            "attempts": attempts,
            "detected": min(attempts, measured["barge_in"]["detected"]) if attempts else measured["barge_in"]["detected"],
            "latency_ms": measured["barge_in"]["latency_ms"],
            "latency_method": measured["barge_in"]["method"],
            "no_interruption_trials": _ask_int("Quante risposte hai lasciato finire senza parlare?"),
            "false_stops": _ask_int("In quante di queste Jake si e' fermato da solo?"),
            "echo_commands": _ask_int("Quante volte Jake ha eseguito un comando uscito dalla SUA voce?"),
        }
        report["streaming"] = {
            "partial_latency_ms": measured["streaming"]["partial_latency_ms"],
            "duplicate_finals": measured["streaming"]["duplicate_finals"],
            "fallback_full_utterance": not measured["streaming"]["partial_latency_ms"],
        }
        report["tts"] = {"first_emission_ms": measured["tts"]["first_emission_ms"]}
    report["wake"] = {
        "hours": measured["hours"],
        "accepted": measured["wake"]["accepted"],
        "false_wakes": _ask_int("Quante attivazioni NON volute hai contato?"),
        "intentional": _ask_int("Quanti 'Jake' intenzionali hai pronunciato (0,5 m / 3 m / 6 m)?"),
        "missed": _ask_int("Quanti di questi Jake NON ha sentito?"),
    }
