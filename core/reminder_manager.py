...
class ReminderManager:
    ...
    `check_same_thread=False` di proposito - `ReminderScheduler` (core/scheduler.py) chiama
    `due_reminders()` ogni pochi secondi da un thread separato, mentre il thread principale puo'
    chiamare `add`/`delete_matching`/`snooze_matching` per un comando diretto nello stesso
    istante. Disattivare quel controllo rende Python felice di condividere la connessione tra
    thread, ma NON la rende automaticamente sicura da usare cosi': la documentazione di sqlite3
    e' esplicita che disattivarlo sposta la responsabilita' di serializzare l'accesso su chi
    chiama. Senza un lock, `delete_matching`/`snooze_matching` (che fanno prima una SELECT via
    `find_matching`, poi una UPDATE/DELETE sull'id trovato) hanno anche una finestra TOCTOU:
    se `due_reminders()` marca quello stesso promemoria `fired=1` proprio in mezzo alle due
    query, l'esito dipende dall'ordine reale di esecuzione, non da cosa l'utente si aspetta.
    Un `RLock` (non un `Lock` semplice) perche' `delete_matching`/`snooze_matching` chiamano
    `find_matching` internamente: con un lock non-rientrante il secondo tentativo di acquisirlo
    nello stesso thread si bloccherebbe per sempre."""
